...
class AbstractCLI(ABC):
    """ Abstract class for simDRIFT command-line interface tools.
    
    Note:
        Tools are called from the command line using
        simDRIFT TOOL-NAME --optional_arg1 optional_val1 ...
...
def generate_cli_dictionary() -> Dict[str, AbstractCLI]:
    ...
    for tool_name in TOOL_NAME_LIST:
        ...
        module_cli_str_list = [tool_name.replace("-", "-"), "cli"]
...
def get_populated_argparser() -> argparse.ArgumentParser:
    ...
    subparsers = parser.add_subparsers(
        title = "sub-commands",
        description = "valid simDRIFT commands",
        dest = "tool")

    for tool_name in TOOL_NAME_LIST:
        module_argparser_str_list = [tool_name.replace("-", "_"), "argparser"]
        module_argparse = importlib.import_module('.'.join(module_argparser_str_list))
