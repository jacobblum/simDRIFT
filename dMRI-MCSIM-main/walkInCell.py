...
def walk_in_cell(spin, cell_info, fiber_xycordinate, TE, dt, spin_pos):    
    ...
    for i in (range(1, int(TE/dt))):
        ...
        # Check to make sure it's within the cell
        if dist_from_cell_origin > cell_info[3]:
            in_water = True 
        # Check to make sure it's not within an xy_fiber
        if any([np.linalg.norm(spin_trajectories[i,0:2]-fibers_xy_nrby[j,0:2], ord = 2) < fibers_xy_nrby[j,5] for j in range(fibers_xy_nrby.shape[0])]):
            in_fiber_xy = True 
        # Check to make sure it's not within a yz_fiber
        ...
        # if any are true, move is invalid 
        if any([in_fiber_xy, in_fiber_zy, in_water]):
            spin_trajectories[i,:] = spin_trajectories[i-1,:]
        
        else:
            spin_trajectories[i,:] = spin_trajectories[i,:]
