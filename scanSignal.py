...
def signal(trajectory_1, trajectory_2, bmax, num_bvals, spin_positions, xyz):
    ...
    nb.save(nifti_dwi, r"Z:\Jacob_Blum\MCSIM-Jacob\signals\CrossingFibers\ABCD\3fibers_2crossing_all" + ".nii")
...
def plot():
    ...
    positions1 = np.load(r"Z:\Jacob_Blum\MCSIM-Jacob\sim_data\from_114\220708_3_vert\spin_positions_3_fibers_50k.npy")
    traj1_1 = np.load(r"Z:\Jacob_Blum\MCSIM-Jacob\sim_data\from_114\220708_3_vert\taj1_3_fibers_50k.npy")
    traj2_1 = np.load(r"Z:\Jacob_Blum\MCSIM-Jacob\sim_data\from_114\220708_3_vert\traj2_3_fibers_50k.npy")

    positions2 = np.load(r"Z:\Jacob_Blum\MCSIM-Jacob\sim_data\from_115\220708_3_vert\spin_positions_3_fibers_50k.npy")
    traj1_2 = np.load(r"Z:\Jacob_Blum\MCSIM-Jacob\sim_data\from_115\220708_3_vert\taj1_3_fibers_50k.npy")
    traj2_2 = np.load(r"Z:\Jacob_Blum\MCSIM-Jacob\sim_data\from_115\220708_3_vert\traj2_3_fibers_50k.npy")

    positions = np.hstack((positions1, positions2))
    traj1 = np.vstack((traj1_1, traj1_2))
    traj2 = np.vstack((traj2_1, traj2_2))
    ...
    if xyz:
        ...
        ax1.plot(b_vals, (sx), 'rx', label = r'$ADC^{x}:$ %s, %s' %(str(round(Dx[1],3)), str(round(e_x,5))))
        ax1.plot(b_vals, (sy), 'bx', label = r'$ADC^{y}:$ %s, %s' %(str(round(Dy[1],3)), str(round(e_x,5))))
        ax1.plot(b_vals, (sz), 'gx', label = r'$ADC^{z}$ %s, %s' %(str(round(Dz[1],3)),  str(round(e_x,5))))
