...
def _find_spin_locations_kernel(resident_fiber_indxs_cuda: numba.cuda.cudadrv.devicearray.DeviceNDArray, 
    ...
    for j in range(fiber_centers_cuda.shape[0]):
        ...
        dynamic_fiber_center = linalg.gamma(spin_positions_cuda[i, :], 
                                            fiber_directions_cuda[j,:], 
                                            theta_cuda[j],
                                            dynamic_fiber_center,
                                            curvature_params[j,:]
                                            )
    
        dynamic_fiber_direction = linalg.d_gamma__d_t(spin_positions_cuda[i, :],
                                                      fiber_directions_cuda[j,:], 
                                                      theta_cuda[j],
        ...
        for k in range(dynamic_fiber_center.shape[0]): 
            dynamic_fiber_center[k] = dynamic_fiber_center[k] + fiber_centers_cuda[j, k]

        dFv = linalg.dL2(spin_positions_cuda[i, :], 
                         dynamic_fiber_center, 
                         dynamic_fiber_direction, 
                         True
                         )
...
def _find_spin_locations(self):
    ...
    theta_cuda                = cuda.to_device(np.array([fiber.theta for fiber in self.fibers], dtype= np.float32))
    curvature_params          = cuda.to_device(np.array([[fiber.__dict__['kappa'], fiber.__dict__['L'], fiber.__dict__['A'], fiber.__dict__['P']] for fiber in self.fibers], dtype = np.float32))
