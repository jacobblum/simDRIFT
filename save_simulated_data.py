...
def _save_data(self, path, plot_xyz):
    ...
    np.save(data_dir + os.sep + "fiberPositionsT1m.npy", self.fiberPositionsT1m)
    np.save(data_dir + os.sep + "fiberPositionsT2p.npy", self.fiberPositionsT2p)
    ...
    nb.save(dwiFiber, data_dir + os.sep + "pureFiberSignal.nii")
    ...
    if self.cellFraction > 0:
        ...
        expSignal, bvals = _signal(self, np.vstack([self.fiberPositionsT1m,self.extraPositionsT1m]), np.vstack([self.fiberPositionsT2p,self.extraPositionsT2p]), xyz = False, finite = False)
